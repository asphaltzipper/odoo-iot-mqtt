...
class MQTTBroker(models.Model):
    ...
    def action_renew_broker(self):
        ...
        for broker in self:
            try:
                ...
                if broker.state == 'draft' and client:
                    _logger.error(f"The broker {broker.name} not in draft state.")
                    raise UserError(f"The broker {broker.name} not in draft state.")
    ...
    def action_connection(self):
        ...
        for broker in self:
            try:
                client = broker._get_connected_client()
                if broker.state == 'connect' and client:
    ...
    def action_disconnect(self):
        ...
        for broker in self:
            try:
                client = broker._get_connected_client()
                if broker.state == 'disconnect' and client:
        ...
        def on_message(client, userdata, msg):
            ...
            with registry(dbname).cursor() as cr:
                ...
                sub_exist = env['mqtt.subscription'].search([
                    ('topic_id', '=', topic.id if topic else False),('state', '=', 'subscribe')
                ], limit=1)
                if not sub_exist:
